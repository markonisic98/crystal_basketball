...
from dataCollection.scrapers import get_game_odds, get_team_game_log
from datetime import datetime
from calendar import month_abbr
from dataCollection.helpers import (create_game_ID, cum_win_loss, win_percentage_moving_avg,
...
def get_game_log_and_odds(team, season_end_year, moving_avgs_list, print_progress=True,
                         add_upcoming=False, date_upcoming=datetime.today().strftime('%Y-%m-%d')):
    df = get_team_game_log(team, season_end_year, moving_avgs_list, add_upcoming=add_upcoming, 
...
def create_final_dataset(season_end_year, moving_avgs_list, first_game_to_collect=1, last_game_to_collect=82,
    ...
    for team in TEAM_TO_TEAM_ABBR.values():
        team_dataset = get_game_log_and_odds(team, season_end_year, moving_avgs_list,
                                            add_upcoming=add_upcoming, date_upcoming=date_upcoming)
...
# function to get the season gamelog from the current season, if it is currently the second year in the 2-year
# season titel (e.g. 2016, while in the 2015/16 season). If it is the first of the two years, manually change the 
# season_end_year parameter to the second of the two years in the current season.
def get_live_season_gamelog(moving_avgs_list, season_end_year=int(datetime.today().strftime('%Y'))):
    
    current_season = create_final_dataset(season_end_year, moving_avgs_list, add_upcoming=True)
